...
tags: [enumeration, ftp, smb, nfs, dns, smtp, snmp, mysql, mssql, oracle, ssh, rdp, winrm]
---
...
⚠ Check `no_root_squash` in `/etc/exports` — see [Linux Privesc → NFS root squashing](../4-post-exploitation/01-linux-privilege-escalation.rmd).

## DNS (53)

See [DNS Reconnaissance](./02-dns-recon.rmd).
...
Goal: basic enum, then if the SQL server is privileged → run `xp_cmdshell`. See [Windows Privesc → SQL → SeImpersonate](../4-post-exploitation/02-windows-privilege-escalation.rmd).
...
## See Also
- [Nmap](./03-nmap.rmd) — for the initial port discovery.
- [Active Directory Enumeration](./06-active-directory-enumeration.rmd) — once you've found SMB/LDAP/Kerberos on a DC.
- [Password Attacks](../2-pre-exploitation/03-password-attacks.rmd) — for spraying credentials against discovered services.
